...
var COMPLAINTS_DB = (function () {
  ...
  /**
   * Flag the driver's account without suspending them.
   * Complaint is marked approved with actionType:'flagged'.
   */
  function flagAccount(id, officerName, officerId, barangay, reason) {
    var items = _getAll();
    var item  = null;
    for (var i = 0; i < items.length; i++) {
      if (items[i].id === id) { item = items[i]; break; }
    ...
    if (item.driverId && typeof ZC_DB !== 'undefined') {
      ZC_DB.flagDriver(item.driverId, officerName, officerId, reason || 'Complaint verified by officer', id);
    }
    ...
    if (typeof NOTIFS_DB !== 'undefined') {
      NOTIFS_DB.add({
        ...
        type:        'complaint-approved',
        reportId:    item.reportId,
        title:       'Your complaint has been reviewed',
        desc:        'Your problem report for Report #' + item.reportId + ' was reviewed by ' +
                     (officerName || 'the barangay officer') +
                     '. The driver\'s account has been flagged with a warning.',
        tag:         'Flagged',
        tagColor:    'red',
        icon:        'fa-flag',
        iconColor:   'red'
    ...
    _logAction({
      ...
      officerName: officerName,
      barangay:    barangay || item.barangay,
      action:      'complaint_driver_flagged',
      targetId:    id,
      details:     'Flagged driver ' + item.driverName + ' account on complaint ' + id +
                   ' (report ' + item.reportId + '). Reason: ' + (reason || 'none')
    });
