...
function validatePanel2() {
  var street   = document.getElementById('regStreet');
  var barangay = document.getElementById('regBarangay');
  var city     = document.getElementById('regCity');
  var province = document.getElementById('regProvince');
  var zip      = document.getElementById('regZip');
  var ok = true;

  [street, barangay, city, province, zip].forEach(function (el) {
    if (!el) return;
    if (!el.value.trim()) { el.style.borderColor = '#dc2626'; ok = false; }
    else el.style.borderColor = '';
  });
...
function checkEmailExists(email) {
  var user = ZC_DB.findByEmail(email);
  return user !== null;
}

function showErrorPanel1(msg) {
  var errorContainer = document.getElementById('panel1-error');
  if (!errorContainer) {
    // Create error container if it doesn't exist
    var panel1 = document.getElementById('panel1');
    var errorDiv = document.createElement('div');
    errorDiv.id = 'panel1-error';
    errorDiv.style.cssText = 'display:none;color:#dc2626;font-size:13px;margin-bottom:8px;font-weight:500;';
    errorDiv.innerHTML = '<i class="fa-solid fa-circle-exclamation"></i> <span id="panel1-error-text"></span>';
    panel1.insertBefore(errorDiv, panel1.querySelector('.btn-row'));
    errorContainer = errorDiv;
  }
  var errorText = document.getElementById('panel1-error-text');
  if (errorText) errorText.textContent = msg;
  if (errorContainer) errorContainer.style.display = 'block';
}

function hideErrorPanel1() {
  var errorContainer = document.getElementById('panel1-error');
      ...
      if (document.getElementById('radio1').checked) {
        ...
        if (validatePanel1()) {
          ...
          if (checkEmailExists(email)) {
            showErrorPanel1('An account with this email already exists. Please sign in or use a different email.');
          } else {
            hideErrorPanel1();
            // Proceed to Panel 2 (don't show OTP yet)
            document.getElementById('radio2').checked = true;
          }
        }
      }
