...
function renderFilterBar() {
  ...
  const filters = [
    { key: 'All',      icon: '' },
    { key: 'Pending',  icon: '<i class="fa-solid fa-hourglass-half"></i> ' },
    { key: 'Approved', icon: '<i class="fa-solid fa-circle-check"></i> ' },
    { key: 'Rejected', icon: '<i class="fa-solid fa-circle-xmark"></i> ' }
  ];
  bar.innerHTML = filters.map(f =>
    `<button class="tab-label${currentFilter === f.key ? ' tab-active' : ''}" onclick="setFilter('${f.key}')">
      ${f.icon}${f.key} (${counts[f.key]})
    </button>`
  ).join('');
}
...
function render() {
  ...
  const statuses = currentFilter === 'All' ? ['Pending', 'Approved', 'Rejected'] : [currentFilter];
  let html = '';

  statuses.forEach(function(status) {
    const apps = driverApplications.filter(function(a) { return a.status === status; });
...
function approveApplication(id) {
  ...
  if (!app) return;
  if (window.confirm('Are you sure you want to verify ' + app.name + ' as a Driver?')) {
    app.status = 'Approved';
...
function rejectApplication(id) {
  ...
  if (window.confirm("Are you sure you want to reject " + app.name + "'s application?")) {
    ...
    app.rejectionReason = 'Rejected by officer';
    render();
  }
}
