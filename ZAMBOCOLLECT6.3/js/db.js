...
var ZC_DB = (function () {
  ...
  /** Save the logged-in user to sessionStorage (password excluded). */
  ...
  function approveNameChangeRequest(data) {
    ...
    if (data.officerId && request.userId === data.officerId) {
      return { ok: false, error: 'You cannot approve your own name change request.' };
    }
    ...
    if (user) {
      ...
      saveUsers(users);
      var cur = getSession();
      if (cur && cur.id === user.id) setSession(user);
  ...
  function rejectNameChangeRequest(data) {
    ...
    if (!request) return { ok: false, error: 'Request not found.' };
    if (data.officerId && request.userId === data.officerId) {
      return { ok: false, error: 'You cannot action your own name change request.' };
    }
  ...
  /**
   * Add a flag/warning to a driver's account (visible on their profile).
   * Does NOT suspend — use suspendDriver for full suspension.
   * @param {string} driverId
   * @param {string} officerName
   * @param {string} officerId
   * @param {string} reason
   ...
   */
  function flagDriver(driverId, officerName, officerId, reason, complaintId) {
    var users = getUsers();
    for (var i = 0; i < users.length; i++) {
      if (users[i].id === driverId) {
        if (!users[i].flags) users[i].flags = [];
        users[i].flags.push({
          id:          'FLG-' + Date.now(),
          flaggedBy:   officerName  || 'Officer',
          flaggedById: officerId    || '',
          flaggedAt:   new Date().toISOString(),
          reason:      reason       || 'Complaint verified by officer',
          complaintId: complaintId  || ''
        });
        users[i].flagCount = users[i].flags.length;
        saveUsers(users);
        var session = getSession();
        if (session && session.id === driverId) setSession(users[i]);
        _logOfficerAction({
          ...
          targetId:    driverId,
          details:     'Driver ' + (users[i].firstName + ' ' + users[i].lastName).trim() +
                       ' account flagged. Reason: ' + (reason || 'Complaint verified')
        });
        if (typeof NOTIFS_DB !== 'undefined') {
          NOTIFS_DB.add({
            driverId:   driverId,
            ...
            title:      'A warning flag has been added to your account',
            desc:       'Officer ' + (officerName || 'a barangay officer') + ' flagged your account. ' +
                        'Reason: ' + (reason || 'Complaint verified by officer') +
                        '. This warning is now visible on your driver profile.',
            tag:        'Flagged',
            tagColor:   'red',
            icon:       'fa-flag',
            iconColor:  'red'
