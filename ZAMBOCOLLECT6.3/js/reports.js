...
var REPORTS_DB = (function () {
  ...
  function add(data) {
    ...
    var report = {
      ...
      status:      'pending',
      incidentId:  null   // will be set by INCIDENTS_DB.findOrCreate()
    };
    reports.unshift(report); // newest first
    _save(reports);

    // ── Group into an incident (or create a new one) ─────────
    // incidents.js must be loaded after reports.js for this to work.
    if (typeof INCIDENTS_DB !== 'undefined') {
      try { INCIDENTS_DB.findOrCreate(report); } catch (e) {}
    }
   ...
   * @param {string|null} collectionPhoto  — base64 data URI proof photo taken by driver
   */
  function collect(id, driverName, collectionPhoto) {
    var r = _update(id, {
      status:          'collected',
