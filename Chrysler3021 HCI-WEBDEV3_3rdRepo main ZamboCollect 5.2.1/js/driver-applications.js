...
function saveApplicationsToStorage() {
  try {
    ...
    // Update stored applications with current data
    driverApplications.forEach(function(currentApp) {
      var idx = stored.findIndex(function(s) { return s.id === currentApp.id || s.applicantName === currentApp.name; });
      if (idx !== -1) {
...
function render() {
  ...
  if (currentTab === 'Applications') {
    if (currentFilter === 'All') {
      apps = driverApplications.filter(function(a) { return a.status === 'Pending' || a.status === 'Rejected'; });
    } else {
      apps = driverApplications.filter(function(a) { return a.status === currentFilter; });
    }
  } else {
...
function approveApplication(id) {
  ...
  confirmBtn.onclick = function() {
    ...
    // Auto-assign in ZC_DB if user exists
    if (app.email) {
      try {
        const users = JSON.parse(localStorage.getItem('zc_users')) || [];
        const idx = users.findIndex(u => u.email.toLowerCase() === app.email.toLowerCase());
        if (idx !== -1) {
  ...
  confirmBtn.onclick = function() {
    ...
    if (typeof NOTIFS_DB !== 'undefined') {
      NOTIFS_DB.add({
        ...
        desc:        'Your driver application has been reviewed and rejected. Reason: ' + reason + '. You may reapply after addressing the feedback.',
        tag:         'Rejected',
        tagColor:    'red',
